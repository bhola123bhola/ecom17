...
export class UserProfileComponent implements OnInit{
  ...
  ngOnInit(): void {
    ...
    if (userSessionId) {
      this.user_id = userSessionId; // Assign as a string
    } else {
      console.error('User session ID is not set in sessionStorage.');
    }
  ...
  editUserData(user_id:any){
    ...
    if (!user_id) {
      console.error('Invalid user ID: Cannot fetch user data.');
      return;
    }
